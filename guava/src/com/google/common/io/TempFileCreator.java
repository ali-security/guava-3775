...
/**
 * Creates temporary files and directories whose permissions are restricted to
 * the current user or,
 * in the case of Android, the current app. If that is not possible (as is the
 * case under the very
 * old Android Ice Cream Sandwich release), then this class throws an exception
 * instead of creating
...
abstract class TempFileCreator {
     ...
     * @throws UnsupportedOperationException if the system does not support creating
     *                                       temporary
    ...
    private static TempFileCreator pickSecureCreator() {
        ...
        try {
            ...
            /*
             * I assume that this check can't fail because JELLY_BEAN will be present only
             * if we're
            ...
            // Don't merge these catch() blocks, let alone use ReflectiveOperationException
            // directly:
            // b/65343391
        } catch (NoSuchFieldException e) {
            // The JELLY_BEAN field doesn't exist because we're running on a version before
            // Jelly Bean :)
            return new ThrowingCreator();
        } catch (ClassNotFoundException e) {
            // Should be impossible, but we want to return *something* so that class init
            // succeeds.
            return new ThrowingCreator();
    ...
    private static final class JavaNioCreator extends TempFileCreator {
        private static final FileAttribute<Set<PosixFilePermission>> RWX_USER_ONLY = PosixFilePermissions
                .asFileAttribute(PosixFilePermissions.fromString("rwx------"));
        private static final FileAttribute<Set<PosixFilePermission>> RW_USER_ONLY = PosixFilePermissions
    ...
    private static final class ThrowingCreator extends TempFileCreator {
        private static final String MESSAGE = "Guava cannot securely create temporary files or directories under SDK versions before"

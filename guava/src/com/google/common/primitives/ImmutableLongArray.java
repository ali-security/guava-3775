 ...
 * {@code Collection}, or
 * {@code List} (though the most common utilities do have replacements here, and
 * there is a
...
public final class ImmutableLongArray implements Serializable {
     ...
     *
     * <p>
     * <b>Performance note:</b> this method delegates to {@link #copyOf(Collection)}
     * if {@code
     * values} is a {@link Collection}. Otherwise it creates a {@link #builder} and
     * uses {@link
     * Builder#addAll(Iterable)}, with all the performance implications associated
     * with that.
     */
    ...
    /**
     * Returns a new, empty builder for {@link ImmutableLongArray} instances, sized
     * to hold up to
     * {@code initialCapacity} values without resizing. The returned builder is not
     * thread-safe.
     *
     * <p>
     * <b>Performance note:</b> When feasible, {@code initialCapacity} should be the
     * exact number
     * of values that will be added, if that knowledge is readily available. It is
     * better to guess a
     * value slightly too high than slightly too low. If the value is not exact, the
     * {@link
     * ImmutableLongArray} that is built will very likely occupy more memory than
     * strictly necessary;
     ...
     *
     * <p>
     * <b>Performance note:</b> The {@link ImmutableLongArray} that is built will
     * very likely
    ...
    public static final class Builder {
         ...
         *
         * <p>
         * <b>Performance note:</b> the returned array is backed by the same array as
         * the builder, so
         * no data is copied as part of this step, but this may occupy more memory than
         * strictly
         * necessary. To copy the data to a right-sized backing array, use
         * {@code .build().trimmed()}.
         */
     ...
     * fields from most
     * instances. Note that the instances that would get smaller are the right set
     * to care about
     ...
     * footprint as this one
     * does (no actual copying is performed). To reduce memory usage, use
     * {@code subArray(start,
     * end).trimmed()}.
     */
     ...
     * note that {@code
     * long} values are boxed into {@link Long} instances on demand, which can be
     * very expensive. The
     * returned list should be used once and discarded. For any usages beyond that,
     * pass the returned
     * list to {@link com.google.common.collect.ImmutableList#copyOf(Collection)
     * ImmutableList.copyOf}
     * and use that list instead.
     */
    public List<Long> asList() {
        /*
         * Typically we cache this kind of thing, but much repeated use of this view is
         * a performance
         * anti-pattern anyway. If we cache, then everyone pays a price in memory
         * footprint even if
    ...
    static class AsList extends AbstractList<Long> implements RandomAccess, Serializable {
        ...
        // Because we happen to use the same formula. If that changes, just don't
        // override this.
        @Override
        public int hashCode() {
    ...
    /**
     * Returns {@code true} if {@code object} is an {@code ImmutableLongArray}
     * containing the same
    ...
    /**
     * Returns an immutable array containing the same values as {@code this} array.
     * This is logically
     * a no-op, and in some circumstances {@code this} itself is returned. However,
     * if this instance
     * is a {@link #subArray} view of a larger array, this method will copy only the
     * appropriate range
